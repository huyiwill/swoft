 ...
 * @Breaker("role")
 */
class RoleBreaker extends CircuitBreaker
{
    /**
     * The number of successive failures
     * If the arrival, the state switch to open
     *
     * @Value(name="${config.breaker.user.failCount}", env="${ROLE_BREAKER_FAIL_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.user.successCount}", env="${ROLE_BREAKER_SUCCESS_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.user.delayTime}", env="${ROLE_BREAKER_DELAY_TIME}")
     * @var int
     */
